...
if uniqid>"" then
  ...
  lawid = ""
  sortid = 1
  sqlstr = "select max(sortid) from weblaw where lawcatego=2"
  set rs = objconn.execute(sqlstr)
  if not rs.eof then
    if trim(rs(0))>"" then
      sortid = cdbl(rs(0)) + 1
...
<form id="myform" onsubmit="return false;" enctype="multipart/form-data">
  ...
  <input type="hidden" name="lawcatego" value="2">
  ...
  <div class="form-group row">
    <label for="lawcontent" class="col-sm-2 col-form-label" style="text-align: right;">法令解釋內容：</label>

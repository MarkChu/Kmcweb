...
sqlstr = sqlstr & " where lawcatego=3 " & vbcrlf   
...
%>
<div>
  <div >財政預算法規 中目前有 <font color="red" style="font-weight: bold;"><%=cnt%></font> 項資料.</div>
    ...
    if isarray(data_ary) then
      for rows=0 to ubound(data_ary,2)
        ...
        <tr>
          ...
          <td data-classes="ftRow"><a href="?p=weblaw3_add&uniqid=<%=data_ary(0,rows)%>"><%=data_ary(2,rows)%></a></td>

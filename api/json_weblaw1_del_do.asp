...
if status="0000" then
	...
	sql_cmd.CommandText = "delete from weblawdet where lawid in (select lawid from weblaw where uniqid in ("&sql_uniqid&")) " 
	'ADO.CreateParameter(name,type,direction,size,value)
	set rs = sql_cmd.Execute

	set sql_cmd1 = Server.CreateObject("ADODB.Command") 
	sql_cmd1.ActiveConnection = Objconn
	sql_cmd1.CommandText = "delete from weblaw where uniqid in ("&sql_uniqid&") " 
	'ADO.CreateParameter(name,type,direction,size,value)
	set rs1 = sql_cmd1.Execute
